public class Matrix {
	...
	public static Matrix addMatrix(Matrix first, Matrix other) {
		...
		if (rows == otherRows && cols == otherCols) {
			...
			for (int i = 0; i < rows; i++) {
				for (int j = 0; j < cols; j++) {
					result.setElement(i, j, other.getElement(i, j) + first.getElement(i, j));
				}
			}
			return result;
		}
		...
		if (cols == otherRows) {
			...
			for (int i = 0; i < rows; i++) {
				for (int j = 0; j < otherCols; j++) {
					result.setElement(i, j, dotProduct(first.getRow(i), other.getCol(j)));
				}
			}
			return result;
		}
		...
		if (fRows == sRows && fCols == sCols) {
			for (int i = 0; i < fRows; i++) {
				for (int j = 0; j < fCols; j++) {
					if (first.getElement(i, j) != second.getElement(i, j)) {
						return false;
					}
				}
			}
